...
def _sale_to_read(sale: Sale) -> SaleRead:
    ...
    read.remaining_amount = round(read.total - read.advance_amount, 2)
    return read
...
def list_sales(db: DbSession, params: Annotated[FilterParams, Query()]):
    ...
    return SalePage(
        data=[_sale_to_read(r) for r in rows],
        total=total,
        page=page,
        limit=limit,
