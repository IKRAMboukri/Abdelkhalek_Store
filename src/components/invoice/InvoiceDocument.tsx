...
export function InvoiceDocument({ invoice, className }: InvoiceDocumentProps) {
  ...
  return (
    <div
      className={clsx('invoice-printable bg-white text-black p-4 mx-auto', 'max-w-[210mm]', className)}
    ...
    >
      <div className="flex items-start justify-between border-b-2 border-gray-800 pb-3 mb-3">
        <div className="flex items-center gap-2">
          {invoice.storeLogo ? (
            <img
              ...
              className="w-11 h-11 rounded-lg object-contain border border-gray-200"
            />
          ) : (
          ...
          <div>
            <h1 className="text-lg font-bold text-black">{invoice.storeName}</h1>
        ...
        <div className="text-right">
          <h2 className="text-xl font-bold uppercase tracking-wider text-gray-800">{t('common.invoice')}</h2>
      ...
      <div className="mb-3">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-1">{t('invoices.billTo')}</h3>
        <div className="bg-gray-50 rounded-lg p-2.5 border border-gray-200">
          ...
          {invoice.notes && (
            <p className="text-xs text-gray-500 mt-1.5 border-t border-gray-200 pt-1.5">{invoice.notes}</p>
          )}
        </div>
      </div>
      ...
      <table className="w-full border-collapse mb-3">
        <thead>
          <tr className="border-b-2 border-gray-800">
            <th className="text-left py-1.5 px-1 text-[10px] font-semibold uppercase tracking-wider text-gray-600 w-6">#</th>
            <th className="text-left py-1.5 px-1 text-[10px] font-semibold uppercase tracking-wider text-gray-600">{t('invoices.product')}</th>
            <th className="text-right py-1.5 px-1 text-[10px] font-semibold uppercase tracking-wider text-gray-600 w-12">{t('invoices.quantity')}</th>
            <th className="text-right py-1.5 px-1 text-[10px] font-semibold uppercase tracking-wider text-gray-600 w-20">{t('invoices.unitPrice')}</th>
            <th className="text-right py-1.5 px-1 text-[10px] font-semibold uppercase tracking-wider text-gray-600 w-20">{t('invoices.total')}</th>
          </tr>
        </thead>
        <tbody>
          {invoice.items.map((item, idx) => (
            <tr key={idx} className="border-b border-gray-100">
              <td className="py-1.5 px-1 text-xs text-gray-500">{idx + 1}</td>
              <td className="py-1.5 px-1 text-xs text-black">{item.productName}</td>
              <td className="py-1.5 px-1 text-xs text-right text-black">{item.quantity}</td>
              <td className="py-1.5 px-1 text-xs text-right text-black">DH {item.unitPrice.toFixed(2)}</td>
              <td className="py-1.5 px-1 text-xs text-right font-medium text-black">DH {item.total.toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="border-t-2 border-gray-800 pt-3 mb-3">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-1">{t('invoices.paymentInfo')}</h3>
        <div className="flex justify-between text-xs text-gray-600">
